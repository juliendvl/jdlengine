...
#ifndef NDEBUG
static constexpr bool sUseValidationLayers = true;
#else
static constexpr bool sUseValidationLayers = false;
#endif // !NDEBUG
...
static void s_CheckValidationLayersSupport()
{
    ...
    for (const char* requiredLayer : sValidationLayers)
    {
        ...
        if (!layerFound) {
            JDL_FATAL("Layer {} is not supported", requiredLayer);
        }
    }
}
